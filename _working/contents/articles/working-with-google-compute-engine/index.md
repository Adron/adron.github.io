...
First I logged in and setup a GCE Account ([read specifically about creating and getting started with a GCE account](https://cloud.google.com/compute/docs/quickstart)) and got *gcloud* configured. The *gcloud* is a cli to manage GCE. It's actually a super powerful tool that comes in handy for all sorts of things. Besides managing GCE, it has a thin wrapper around [ssh](https://en.wikipedia.org/wiki/Secure_Shell), [scp](https://en.wikipedia.org/wiki/Secure_copy), and working with servers with those respective tools. I'll use it a bit later to actually run some scripts against the instance I'll be creating.

Once you've signed up for GCE there's a few things worth noting. One is the idea of the *project* that Google uses within GCE. This is something you're create, or rename the default, or in some way bring into existence to use. A *project* is something that a host of instances, instance groups, load balancers, networks, networking, and more can be allocated against. It's also something that can be setup for or inside a specific billing group. It might also be helpful to really get an understanding of what a *project* is by reading the [Google documentation on *projects*](https://cloud.google.com/compute/docs/projects).

<span class="more"></span>

Once the project is ready, we can move forward with installing *gcloud*. The way this is done is by installing the Google Cloud SDK. The curl below pulls down and executes the installation. Then the following command restarts the shell. Finally the gcloud init command kicks off the initialization of the gcloud cli.
...
When building Terraform configurations for Google Cloud there are a number of settings that *gcloud* can pull up very easily. Here are some of the commands I've used most frequently when setting up google compute instances.
...
The last thing is connecting to instances, with *gcloud* looks like the following.

```shell-script
gcloud compute --project "project-name" ssh --zone "us-central1-b" "instance-name"
```

There's some super useful ways to execute commands with the *gcloud* cli, which provides a great way for bash scripting against instances.
...
The next thing we'll need for Terraform use with GCE is the *account.json* file. This is the file that a service account sets up to secure our connection between GCE and Terraform.
...
In the [README.md](https://github.com/Adron/starting-with-gce/blob/master/README.md), of course, we write our documentation! So anyway, it'll be there in the [repo](https://github.com/Adron/starting-with-gce) I've created for this blog entry here.
...
For this first build, I just wanted to get a basic template with a small (and by association super cheap) instance. For this I went with *f1-micro*. For the disk, I used the base disk image load of the *ubuntu-1404-trusty-v20160406*.
